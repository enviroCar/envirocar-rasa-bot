    ...
    def validate_car_number(
        ...
        if metadata["type"] == MetadataType.CAR_SELECTION.value:
            ...
            if slot_value.lower() == "next":
                ...
                # index = None if (next_utter_status["index"] >= 3 or next_utter_status["index"] == 0) else \
                #     next_utter_status["index"]
            ...
            return self.validate_car_selection(dispatcher, slot_value.lower(), cars, select_car_iteration, car_utils, next_car, previous_car, next_index)

    @staticmethod
    def validate_car_selection(dispatcher: CollectingDispatcher, slot_value: str, cars: list, select_car_iteration: int,
                               car_utils: CarUtils, next_car: bool, previous_car: bool, next_index: int) -> Dict[Text, Any]:
        ...
        if next_index != select_car_iteration and next_car:
        ...
        # TODO: new function-> reset all car selection related slots
        if slot_value == "third" and len(available_cars) >= 3:
            dispatcher.utter_message(text=f"{available_cars[2]} car is available")
            return {"car_number": True, "event": "reset_slots", "timestamp": None}
